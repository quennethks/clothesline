...
```mermaid
flowchart TD
    TAM["TAM — Philippines<br/>≈ ₱4.4B/yr<br/>3.5M outsourcing households × ₱1,188<br/>+ 20,000 shops × ₱14,400"]
    SAM["SAM — Metro Manila, app-ready<br/>≈ ₱1.15B/yr<br/>~0.9M smartphone-first households × ₱1,188<br/>+ ~5,500 shops × ₱14,400"]
    SOM["SOM — obtainable, ~3 years<br/>≈ ₱1M–₱26M/yr scenario band<br/>(see scenario table)"]
    TAM --> SAM --> SOM
...
pie showData
    ...
    "Consumer premium subs (Metro Manila)" : 1069
    "Shop-side B2B SaaS (NCR)" : 79
```
...
```mermaid
flowchart TD
    P1["Phase 1 — now<br/>B2C personal tracker, Metro Manila<br/>Free + ₱99/mo premium"] --> P2["Phase 2<br/>Shop directory + reliability ratings<br/>from check-in data<br/>('Which shops lose clothes?')"]
    P2 --> P3["Phase 3 — B2B<br/>Shop-side intake tool: shop confirms<br/>the customer's itemized list at drop-off<br/>₱1,200–3,500/mo per shop"]
    P3 --> P4["Phase 4<br/>Verified-shop network: trust badge,<br/>payments — the PH 'Cents' play"]
