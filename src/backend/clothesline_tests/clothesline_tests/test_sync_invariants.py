...
async def test_editing_count_sent_after_send_is_rejected(
    ...
    await push_create(
        authed_client, "load_item_categories", _category_doc(category_id, load_id)
    )
    ...
    pulled_category = (
        await authed_client.get("/sync/load_item_categories")
    ).json()["documents"][0]
    bad_edit = {**pulled_category, "count_sent": 99}
    conflicts = await push_update(
        authed_client, "load_item_categories", bad_edit, pulled_category
    )
