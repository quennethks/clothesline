...
- [ ] **Byte plumbing we own** (not RxDB): an **upload queue** drains `local_only` bytes to Blob on reconnect (sets `blob_key`), and **lazy cache-on-view** fetches+caches bytes from Blob when a not-local photo is opened online (spec §8.2–8.3).
- [ ] Offline viewing: captured-on-device photos always viewable (bytes kept as local cache); others need a prior online view. Local byte cache is trimmed **least-recently-used first** under storage pressure — but **only uploaded bytes** are evicted, never un-uploaded `local_only` bytes (eager prefetch deferred).
- [ ] Offline capture: bytes stashed locally with the client-only `local_only` flag; the `photos`/`photo_links`/`load_items` **docs** replicate immediately.

**Acceptance:** attach a bundle + a category photo online and offline-then-synced; the category photo creates a `LoadItem`, bumps the auto count (and delete decrements it), appears in the gallery; a photo captured offline is **viewable offline** (local cache) and uploads on reconnect; the `is_primary` photo renders as the load thumbnail; blobs are SAS-gated (not public).
