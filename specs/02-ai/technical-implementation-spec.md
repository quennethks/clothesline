...
- **Domain shift is the top accuracy risk.** Fashionpedia is **fashion/e-commerce/editorial imagery** (garments worn by models, studio lighting), but the app sees **crumpled laundry and hand-held single items in home lighting**. A model trained only on Fashionpedia may generalize poorly — and accuracy is the PRD's headline metric. **Mitigation:** validate against a held-out set of **real present-to-scan photos**, and budget for a small round of **in-domain fine-tuning data** (images captured in the actual scanning setup) if accuracy falls short. Assume this is likely, not a remote contingency.
